"""
Reliability Simulation WITH Repair (MTTR considered)
Calculates: MTBF, MUT, MTTR, Availability
...
def simulate_component(mttf, duty_cycle, mttr, duration, dt):
    """Simulate a single component with repair capability."""
    lam = 1.0 / mttf
...
def run_component_analysis(components_db, duration, dt, n_sims):
    """Run Monte Carlo for each component with repair, calculate MTBF, MUT, MTTR, A."""
...
def run_system_analysis(components_db, duration, dt, n_sims):
    """Run Monte Carlo for the system with repair, calculate MTBF, MUT, MTTR, A."""
...
def create_component_plots(results, output_dir):
    """Create summary plots for component repair analysis."""
...
def create_system_plots(results, output_dir):
    """Create summary plots for system repair analysis."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    ...
    ═══════════════════════════════════════════════
      MTBF: {results['MTBF']:>10.4f} hours
      MUT:  {results['MUT']:>10.4f} hours
      MTTR: {results['MTTR']:>10.4f} hours
      A:    {results['A']:>10.4f} ({results['A']*100:.2f}%)
      λ:    {1/results['MTBF'] if results['MTBF'] != float('inf') else 0:>10.6f} failures/h
    """
    ...
    ax4.pie(sizes, labels=labels, colors=colors, explode=(0.05, 0.05),
            shadow=True, startangle=90, textprops={'fontsize': 11, 'weight': 'bold'})
    ax4.set_title('System Availability')
...
def create_timeline_plot(sample_data, output_dir):
    """Create timeline showing system and component states with repair."""
    ...
    for i in range(len(boundaries) - 1):
        ...
        rect = Rectangle((start_time, y_pos - row_height/2), width, row_height,
                         facecolor=color, edgecolor='darkgray', linewidth=0.8)
        ax.add_patch(rect)
    ...
    for idx, comp_name in enumerate(components):
        ...
        for i in range(len(boundaries) - 1):
            ...
            if width < 0.001:
                continue
            rect = Rectangle((start_time, y_pos - row_height/2), width, row_height,
                             facecolor=color_map[state], edgecolor='none')
            ax.add_patch(rect)
